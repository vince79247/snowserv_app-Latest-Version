...
| 19 | **Provider can get paid without doing the work** | 🔴 | **No verification that a provider was ever at the job.** Reproduced: 4 of 5 timed completions finished in **under 2 minutes** ("0 min on site"), and the emulator provider sitting in **Mountain View, CA** was dispatched + completed a **Yonkers** job. A provider can Accept → Start (**captures the customer's card**) → Complete with any **gallery** photo (#11) → collect 75%, never leaving home. Three holes compound: no geofence, no minimum time on site, no photo authenticity. FIX (highest value first): (1) **geofence Start/Complete to ~150m of `job_lat/lng`** — the data already exists (`providers.current_lat/lng`); (2) camera-only photos (#11); (3) flag sub-N-minute completions for admin review / payout hold. Admin panel already *shows* "0 min on site" (good visibility) but does not *prevent* it. | open (pre-launch) |
