...
| 28 | **`stalled_signup` email is sent to people who cannot log in** | 🟡 | **Found 2026-09-24 auditing why 5 of 7 real providers are stuck at `incomplete`.** None of them had saved a single registration field — equipment, insurance, payouts and agreement were all null for every one. They are not abandoning the 4-step flow; they never reach it. Joining `auth.users` splits them cleanly in two: **three never confirmed their email and have `last_sign_in_at = null`** (Andy Sep 17, James Brennan Sep 13, Taj Anderson Aug 23) — email confirmation is ON (`mailer_autoconfirm=false`), so these three are **locked out of the account they created**; the other two (rosa, scott) confirmed, signed in once, and genuinely stalled in the flow. **The bug:** `send-lead-email` picks its variant from `registration_status`, which reads `incomplete` for both groups, so all three locked-out people were sent **"Finishing your SnowServ provider account"** on 2026-09-18 — an email asking them to go do something they *cannot* do, with no confirmation link in it. That is the one email guaranteed not to convert them. **Not a deliverability failure:** the confirmation mail and the lead mail use the same sender (`noreply@send.snowserv.app`) over the same Resend infrastructure, and DNS is correct (SPF `include:amazonses.com`, DKIM `resend._domainkey` present, DMARC `p=none`). **Fix:** give `send-lead-email` a fourth input — whether `auth.users.email_confirmed_at` is null — and add an `unconfirmed_email` variant carrying a **fresh confirmation link** (Supabase admin `generate_link`, type `signup`), sent instead of `stalled_signup`. Same principle the function already follows: pick the variant from the row's real state. Also surface it in admin: an approved-looking "incomplete" provider who has never signed in should read differently from one who has. | 🟡 open |
| 29 | **Dispatch sent EVERY queued job to the same provider** | 🔴→✅ | **Found 2026-09-24 while planning a 100-customer load test; the load test then proved it.** `dispatch_jobs()` ranks partly by "fewest active jobs", counting `jobs.provider_id = <them> AND status IN ('assigned','in_progress')`. A **pending offer** is neither — it is `status='requested'` with `dispatched_to` set — so an outstanding offer was **invisible to the load ranking**. The dispatcher loops over every queued job in ONE tick, and offering job #1 to somebody changes nothing the ranking reads (`dispatched_to` is not `provider_id`; status stays `requested`), so job #2 evaluated identically and picked the same winner. And #3. And #100. **Measured, 100 jobs / 4 online providers: `scott mosby got 100, everyone else 0, 0 left queued`.** On a storm night that is 100 simultaneous offers on ONE phone, each with its own countdown; they accept one, the other 99 expire together, which adds that provider to all 99 jobs' `rejected_providers`, and the clump cascades onto the next provider at once. **Auto-accept was never affected** — that path sets `status='assigned'` AND `provider_id`, which the count does see; the blind spot was specifically the normal tap-to-accept flow. **FIXED** (migration `20260924190000_dispatch_one_offer_per_provider.sql`): a provider already holding a pending offer is excluded, in BOTH the normal-winner and preferred-driver queries — they can only tap Accept on one. Same test after the fix: **`1 offer each, 96 left queued`**, i.e. a queue instead of a stampede. Dispatch ran in 37ms for 100 jobs, so throughput was never the issue. The expiry loop deliberately stays FIRST in the function so a provider sitting on a timed-out offer is released before the exclusion is evaluated and can never be locked out by a stale one. ⚠️ Accepted trade-off: with one online provider, jobs now go out one per cron tick (every minute) rather than all at once. If that latency ever matters, call `dispatch_jobs()` on accept — do NOT relax the exclusion. **Both runs were executed inside a transaction forced to roll back via `RAISE EXCEPTION`, so no fake job, push, or provider state ever persisted** — verified after: 7 real jobs, 0 queued, only Frank online. | ✅ done + verified live |
