-- CRITICAL FIX: dispatch_jobs() has errored in every version with
--   "operator does not exist: uuid <> text"
-- because `rejected` was declared TEXT[] while jobs.rejected_providers and
-- providers.id are both uuid, so `id != ALL(rejected)` had no matching operator.
-- It went unnoticed while the CLIENT dispatcher (dispatchToNearest) handled new
-- orders; the Stripe-webhook migration made dispatch_jobs() the primary path, so
-- the error started stranding every webhook-created job (auto-accept never
-- fired). Fix: declare `rejected uuid[]` to match the column/id types. All the
-- preferred-driver + load-aware logic is unchanged.
