-- Admin "preferred driver" override (built 2026-07-07). An occasional lever to
-- feed jobs to a specific provider ("take care of a certain driver"): a flagged
-- provider jumps to the FRONT of dispatch ranking. Guardrail: it's just a
-- re-rank on top of the existing eligibility filter, so a preferred provider
-- only wins when they're already ELIGIBLE (online, approved, hasn't declined
-- that job). Offline / declined → the job flows to the next-best provider as
-- usual, so nothing is ever stranded on a favored driver who isn't working.
-- Toggle it on/off per provider in the admin panel.
--
-- Kept in sync with the client dispatcher lib/utils/dispatch.dart (both paths
-- must rank identically — see CLAUDE.md "Dispatch").
...
BEGIN
  ...
  LOOP
    ...
    -- Preferred driver first (admin override), then the load-aware pick:
    -- fewest active jobs, then nearest.
